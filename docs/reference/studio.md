...
| Drag a bar on a plan plane | Moves its dates by whole days (a nested bar stays inside its parent); dragging up or down reorders top-level bars. Drag its left or right edge to change the start or the end. The date under the pointer is where a dropped Zone or Event lands. |
