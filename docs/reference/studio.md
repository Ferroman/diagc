...
| Drag a node past the wall of its group | The group grows around it, in any direction — a drag never takes a node out of its group, except on a plan plane, where a person, team or plain node can be let go over another zone instead of growing this one — see the row below. Also with Alt+drag in view mode. |
...
| Drag a bar on a plan plane | Moves its dates by whole days (a nested bar stays inside its parent); dragging up or down reorders top-level bars. Drag its left or right edge to change the start or the end. The date under the pointer is where a dropped Zone or Event lands (a zone dropped inside another starts where its parent does); dropped without dates yet, it takes them from where it lands instead. A non-plan child scheduled inside a zone (a C4 container, a plain box) drags freely and keeps exactly the spot you leave it at, growing the bar to fit — it never snaps back into the zone's automatic rows. Let go of a person, team or plain node over a zone instead of moving it, and it is assigned the zone's executor role (a person or team) or re-homed there (anything else). |
...
**Plan** — the right dock's own panel on a `plan` plane: zones listed in tree order with their start/end dates and Owner / Executor / Checker pickers, events with their date, and **Add zone**, **Add event**, **Add person** / **Add team** to grow the schedule. Properties gains a matching **Plan** section, shown on a `plan` plane, or anywhere a zone or event already carries dates: a zone's start, end and role pickers, or an event's date. `+` on a selected zone or event adds its successor — the next zone starting the day after this one ends and running the same length, the next event a week later, both clamped inside the parent and linked back with a dependency arrow. Selecting an actor or a zone (both modes, and on a published page) lights up the other side of its roles — the connected zones or actors keep a coloured outline and a solid chip while the rest of the chart dims — the same connected-neighbourhood focus every notation offers, extended here past the role relations that never draw as arrows. Dragging a person or team from the roster onto a zone makes it the zone's executor instead of picking it in the panel (a no-op if it already holds a role on that zone); dragging any other node onto a zone re-homes it under that zone. Clicking one of the bar's own role chips opens a menu to change the actor's role or remove it, in edit mode. See [Draw a plan](../how-to/draw-a-plan.md).
...
**Leverage** — on a plane with the `causal-loop` notation, in view mode, selecting a variable turns the left dock into this panel: the feedback loops the variable sits in, the upstream drivers that reach it with the net sign of each shortest path, and the variables that recur across its loops. `Ctrl`/`Cmd` + click a second variable adds a **Dependency** section. `↻` in the corner controls shows and hides the R/B loop badges; click a badge to light its loop. See [Draw a causal-loop diagram](../how-to/draw-a-causal-loop-diagram.md).

The right dock stacks its panels — the notation's own panel (Git, Activity, And then what?, Fishbone, Threat model or Plan), then **Layout & style**, then **Layers & planes** — in one scrolling column. Click a panel's heading to fold it down to that heading; the others stay open. Which panels are folded is a viewer preference, remembered across reloads and never saved to the diagram. The dock's own chevron still hides the whole column.
...
**Layers & planes** — add, edit and remove layers (id, name, tint) and planes (id, name, containment borrowing, preset layers, its own **Notation**). In edit mode it also carries a model-level **Notation** selector (*default look*, then each notation by name: *Causal loop*, *Git graph*, *C4*, *Second-order thinking*, *Fishbone (cause and effect)*, *Threat model (STRIDE)*, *Plan (schedule)* — a plane's own notation wins where set) and a **Legend** checkbox, which adds or removes the diagram's `legend` declaration; its title, position, sections and items are authored in the file. See [Draw a C4 diagram](../how-to/draw-a-c4-diagram.md) and [Add a legend](../how-to/add-a-legend.md).
