...
A notation can *steer* the automatic layout instead of replacing it. Second-order thinking derives an order per node (the longest path from a decision) and hands it to elk as layer partitions, so the bands you see are elk's own rows — routing, labels and saved positions all keep working exactly as they do for any layered diagram. That is a different seam from git-graph, whose `layout` computes lanes and columns itself and replaces elk entirely. Fishbone takes the same seam: its `layout` draws the spine, bones and ticks from the tree, and elk never sees the diagram. The plan notation goes furthest along it: `planLayout` turns each zone's dates into an x and a width, so the arrangement is not a choice the layout makes but a fact the model already carries. Threat model sits at the far end from both — like C4 it is a stencil notation, a set of registry shapes plus one colour rule for the trust boundary, and elk arranges it exactly as it arranges anything else; what the notation adds is derived rather than drawn, since which flows cross a boundary falls out of the containment the layout was already given.
